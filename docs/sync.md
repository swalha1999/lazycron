...
This creates `.lazycron/config.yaml` (project metadata) and runs `npx sandcastle init` to scaffold `.sandcastle/` (Dockerfile, env, lib helpers).
...
5. **Build the sandcastle image** (`npx sandcastle build-image`) on the target. Cheap on rerun (Docker layer cache). `--no-build` opts out.
